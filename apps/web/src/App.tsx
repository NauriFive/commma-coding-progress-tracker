...
const CHART = [
  ...
  140, 145, 140, 130, 120, 110, 100, 95, 100, 110, 120, 130, 135, 130, 120,
  105, 90, 75, 60, 50, 45, 50, 65, 80,
];

const LEADERS = [
  { rank: 1, handle: "falsetto", sub: "Berlin", time: "42:18:04", lang: "Rust", langDot: "#FF4D1A", streak: 87 },
  { rank: 2, handle: "lumen.dev", sub: "Lisbon", time: "38:51:22", lang: "TypeScript", langDot: "#9CF76D", streak: 54 },
  { rank: 3, handle: "northbound", sub: "Oslo", time: "35:47:09", lang: "Go", langDot: "#7DD3FC", streak: 211 },
  { rank: 4, handle: "yoursquid", sub: "you", time: "32:12:55", lang: "Python", langDot: "#EFEAD8", streak: 12, self: true },
  { rank: 5, handle: "inkpaper", sub: "Tokyo", time: "30:44:01", lang: "Swift", langDot: "#FCA5A5", streak: 31 },
  { rank: 6, handle: "aprilsink", sub: "Mexico City", time: "28:18:33", lang: "Elixir", langDot: "#C4B5FD", streak: 9 },
];
...
function buildChartPath(data: number[], w: number, h: number, pad = 4) {
  ...
  return data
    .map((v, i) => `${i === 0 ? "M" : "L"} ${(pad + i * stepX).toFixed(2)} ${(h - pad - v * scaleY).toFixed(2)}`)
    .join(" ");
}
      ...
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center h-16">
          ...
          <div className="hidden md:flex gap-7 font-mono text-[12px] tracking-wider text-ink-soft">
            {["Activity", "Leaderboards", "Profile", "Pricing", "Changelog"].map((l) => (
              <a
                key={l}
                href="#"
          ...
          <div className="justify-self-end flex items-center gap-3">
            ...
            <a
              href="#"
              className="group inline-flex items-center gap-2.5 h-[38px] px-4 rounded-full font-mono text-[12px] uppercase tracking-wider font-medium
...
function LiveDot({ color = "live" }: { color?: "live" | "accent" }) {
  ...
  return <span className={`w-[7px] h-[7px] rounded-full ${cls} animate-pulse-dot`} />;
}

function Hero() {
  const [pulse, setPulse] = useState(14233);
  useEffect(() => {
    const id = setInterval(() => setPulse((n) => n + Math.floor(Math.random() * 4) + 1), 1700);
  ...
  return (
    <section className="relative">
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)] pt-[clamp(56px,9vw,120px)] pb-[clamp(48px,6vw,96px)]">
        <div
          className="flex items-center gap-3.5 mb-12 opacity-0 [animation:rise_700ms_100ms_ease_forwards]"
        >
          <LiveDot />
        ...
        <h1 className="font-serif font-normal text-[clamp(56px,10.5vw,168px)] leading-[0.92] tracking-[-0.035em] m-0">
          <span className="block overflow-hidden reveal-clip">
            <span className="[animation:rise_900ms_120ms_ease_forwards] [transform:translateY(110%)] inline-block">
              Every commit
            </span>
          </span>
          <span className="block overflow-hidden reveal-clip">
            <span className="[animation:rise_900ms_240ms_ease_forwards] [transform:translateY(110%)] inline-block">
              is a <em className="not-italic font-serif italic text-accent">step.</em>
            </span>
          </span>
        </h1>

        <div className="grid md:grid-cols-[1.4fr_1fr] gap-[clamp(40px,6vw,96px)] items-end mt-[clamp(40px,6vw,80px)]">
          <div>
            <p
              className="font-sans text-[clamp(16px,1.3vw,19px)] leading-relaxed text-ink-soft max-w-[46ch] m-0 mb-8
              opacity-0 [animation:rise_700ms_520ms_ease_forwards]"
            >
              commma turns your editor into a logbook. Pace, splits, streaks, podiums —
              all the rituals of a real sport, for the work you already do. Install once,
              code as usual, then look back at the week like an athlete reviewing the tape.
            </p>
            <div
              className="flex gap-3 items-center flex-wrap opacity-0 [animation:rise_700ms_640ms_ease_forwards]"
            >
              <a
                href="#"
          ...
          <div className="opacity-0 [animation:rise_800ms_720ms_ease_forwards]">
            <div className="grid grid-cols-2 border-t border-rule">
              <div className="py-5 pr-6 border-b border-r border-rule">
                <span className="block font-serif text-[clamp(36px,4vw,56px)] leading-none tracking-[-0.02em] tnum">
              ...
              <div className="py-5 pl-6 border-b border-rule">
                <span className="block font-serif text-[clamp(36px,4vw,56px)] leading-none tracking-[-0.02em] tnum">
                  211<span className="text-ink-mute text-[0.5em] align-baseline">d</span>
                </span>
                <span className="block font-mono text-[11px] tracking-[0.14em] uppercase text-ink-mute mt-2.5">
                  longest streak
                </span>
              ...
              <div className="py-5 pl-6 border-b border-rule">
                <span className="block font-serif text-[clamp(36px,4vw,56px)] leading-none tracking-[-0.02em] tnum">
                  92<span className="text-ink-mute text-[0.5em] align-baseline">%</span>
                </span>
                <span className="block font-mono text-[11px] tracking-[0.14em] uppercase text-ink-mute mt-2.5">
                  return next-day
                </span>
...
function SectionHead({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-8 items-baseline pb-[clamp(40px,5vw,72px)]">
      <span className="font-mono text-[11px] tracking-[0.18em] uppercase text-ink-mute">
        § {no}
      </span>
      <h2 className="font-serif font-normal text-[clamp(32px,4.5vw,64px)] leading-none tracking-[-0.025em] m-0">
        {title}
      </h2>
      {aside && (
...
function ActivityCard() {
  ...
  return (
    <div className="relative border border-rule-strong bg-gradient-to-b from-paper-2 to-paper rounded overflow-hidden">
      {/* head */}
      <div className="grid grid-cols-[1fr_auto] gap-6 items-start px-8 py-7 border-b border-rule">
        <div>
          ...
          <h3 className="font-serif text-[clamp(28px,3vw,42px)] leading-[1.05] tracking-[-0.02em] m-0 text-ink">
            {SESSION.title}
            <span className="block italic text-ink-mute text-[0.6em] mt-1.5">{SESSION.subtitle}</span>
          </h3>
        </div>
        <div className="font-mono text-[12px] text-ink-mute flex flex-col items-end gap-1.5 whitespace-nowrap">
          <span><strong className="text-ink font-medium">{SESSION.date}</strong></span>
          <span>started <strong className="text-ink font-medium tnum">{SESSION.startedAt}</strong></span>
          <span>kudos <strong className="text-ink font-medium tnum">· 18</strong></span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 border-b border-rule">
        {[
          ...
          { label: "Lines moved", value: SESSION.lines.toLocaleString(), unit: "loc", delta: "+12%" },
          { label: "Pace", value: SESSION.pace.toString(), unit: "char/min", delta: null },
          { label: "Languages", value: "4", unit: "", delta: null },
        ].map((s, i) => (
          <div
            key={s.label}
            className={`px-8 py-6 ${i < 3 ? "md:border-r" : ""} border-rule ${i < 2 ? "border-b md:border-b-0" : ""} ${i === 1 ? "border-r md:border-r" : ""}`}
          >
            <span className="block font-mono text-[10.5px] tracking-[0.16em] uppercase text-ink-mute mb-3.5">
              {s.label}
            </span>
            <span className="flex items-baseline gap-1.5 font-serif text-[clamp(30px,3vw,44px)] leading-none tracking-[-0.02em] text-ink tnum">
              {s.value}
              {s.unit && (
      ...
      <div className="px-8 py-8">
        <div className="flex justify-between items-baseline mb-4 font-mono text-[11px] tracking-[0.14em] uppercase text-ink-mute">
          <span>Keystrokes / minute</span>
          <span>peak <span className="text-ink tnum">241</span> at <span className="text-ink tnum">09:51</span></span>
        </div>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[180px] block" preserveAspectRatio="none">
          <defs>
            ...
            <pattern id="cgrid" width="50" height="40" patternUnits="userSpaceOnUse">
              <path d="M50 0 L0 0 0 40" fill="none" stroke="#221f1a" strokeWidth="0.5" />
            </pattern>
          </defs>
        ...
        <div className="grid grid-cols-8 mt-2.5 font-mono text-[10px] text-ink-faint tracking-wider">
          {["08:42", "09:00", "09:20", "09:40", "10:00", "10:20", "10:40", "11:00"].map((t) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border-t border-rule">
        <div className="px-8 py-6 md:border-r border-rule">
          ...
          {SESSION.langs.map((l) => (
            ...
            >
              <span className="w-2.5 h-2.5 rounded-sm" style={{ background: l.swatch }} />
              <span><strong className="text-ink font-medium">{l.name}</strong></span>
              <span className="tnum">{l.time}  ·  {l.pct}%</span>
            </div>
          ))}
        </div>
  ...
  return (
    <section className="py-[clamp(64px,9vw,140px)]">
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        <SectionHead
          no="01"
          title={<>An <em className="italic text-accent">activity</em>, not a screenshot.</>}
...
function HowItWorks() {
  ...
  return (
    <section className="py-[clamp(64px,9vw,140px)] border-t border-rule">
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        <SectionHead
          no="02"
          title={<>Three pieces. <em className="italic text-accent">No fuss.</em></>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 border-y border-rule">
          {steps.map((s, i) => (
            <div
              key={s.no}
              className={`px-8 py-10 ${i < 2 ? "md:border-r" : ""} ${i < 2 ? "border-b md:border-b-0" : ""} border-rule`}
            >
              <div className="font-mono text-[11px] tracking-[0.16em] text-ink-mute flex items-center gap-3 mb-7">
                <span>{s.no}</span>
                <span className="flex-1 h-px bg-rule" />
              </div>
              <pre className="font-mono text-[12px] leading-snug text-accent bg-paper-2 border border-rule px-4 py-3.5 rounded-[2px] mb-6 whitespace-pre">
{s.ascii}
              </pre>
  ...
  return (
    <section className="py-[clamp(64px,9vw,140px)] border-t border-rule">
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        <SectionHead
          no="03"
          title={<>This week's <em className="italic text-accent">podium.</em></>}
          aside="updated · 14 min ago"
        />
        <div className="border-y border-rule-strong">
          {/* head row */}
          <div className="grid grid-cols-[56px_1.4fr_1fr_1fr_1fr_0.6fr_32px] items-center px-6 py-3.5 font-mono text-[10.5px] tracking-[0.14em] uppercase text-ink-mute bg-paper-2 border-b border-rule">
            <span>Rank</span>
            <span>Athlete</span>
          ...
          {LEADERS.map((row) => (
            <div
              key={row.handle}
              className={`group grid grid-cols-[56px_1.4fr_1fr_1fr_1fr_0.6fr_32px] items-center px-6 py-4 border-b border-rule last:border-b-0 font-mono text-[13px] text-ink-soft relative transition-colors
                ${row.self ? "bg-accent-soft" : "hover:bg-paper-2"}`}
            >
              {row.self && <span className="absolute left-0 top-0 bottom-0 w-0.5 bg-accent" />}
              <span className={`font-serif text-[22px] tracking-[-0.02em] tnum ${row.rank === 1 ? "text-accent" : "text-ink"}`}>
                {row.rank.toString().padStart(2, "0")}
              </span>
              <span className="flex items-center gap-3.5">
                <span className="w-8 h-8 rounded-full bg-paper-3 border border-rule-strong font-serif text-[14px] text-ink grid place-items-center">
                  {row.handle[0].toUpperCase()}
                </span>
                <span>
                  <span className="block text-ink font-medium">@{row.handle}</span>
                  <span className="block text-[11px] text-ink-mute mt-0.5">{row.sub}</span>
                </span>
              </span>
              <span className="tnum text-ink">{row.time}</span>
              <span className="inline-flex items-center gap-2 text-[12px]">
                <span className="w-2 h-2 rounded-sm" style={{ background: row.langDot }} />
                {row.lang}
              </span>
              ...
              <span className="text-[11px] text-ink-mute uppercase tracking-wider">{row.sub}</span>
              <span className="text-accent font-mono text-[14px] text-right opacity-0 -translate-x-1 transition group-hover:opacity-100 group-hover:translate-x-0">
                →
              </span>
            </div>
          ))}
...
function Final() {
  return (
    <section className="py-[clamp(80px,12vw,200px)] text-center border-t border-rule relative">
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        <p className="font-mono text-[12px] tracking-[0.18em] uppercase text-ink-mute m-0 mb-9">
          <LiveDot /> &nbsp; 2,841 athletes logging right now
        </p>
        <h2 className="font-serif font-normal text-[clamp(48px,9vw,140px)] leading-[0.95] tracking-[-0.035em] m-0 mb-10 mx-auto max-w-[14ch] text-ink">
          Stop coding into the <em className="italic text-accent">void.</em>
        </h2>
        <form
          onSubmit={(e) => e.preventDefault()}
          className="inline-flex items-center gap-2 border border-rule-strong rounded-full p-1.5 pl-6 bg-paper-2 min-w-[min(440px,90vw)]"
        >
          <input
            type="email"
            placeholder="you@dev.local"
            className="flex-1 bg-transparent border-0 outline-none text-ink font-mono text-[13px] py-3"
          />
          <button
            type="submit"
            className="group inline-flex items-center gap-2.5 h-[42px] px-5 rounded-full font-mono text-[12px] uppercase tracking-wider font-medium bg-accent text-paper border border-accent hover:bg-ink hover:border-ink transition-colors"
          >
            Request access
            <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
          </button>
      ...
      <div className="mx-auto max-w-[1320px] px-[clamp(20px,4vw,56px)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono text-[11px] uppercase tracking-wider text-ink-mute pt-6 border-t border-rule">
          <div className="space-x-5">
            <a href="#" className="hover:text-ink">Activity</a>
            <a href="#" className="hover:text-ink">Leaderboards</a>
            <a href="#" className="hover:text-ink">Profile</a>
          </div>
